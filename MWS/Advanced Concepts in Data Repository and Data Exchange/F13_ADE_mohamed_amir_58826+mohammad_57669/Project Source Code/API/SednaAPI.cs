/*	Sedna .Net API - For use with the Sedna Native XML Server (http://www.modis.ispras.ru/Development/sedna.htm)
	...
	public class SednaSession
	{
		...
		private SednaSession(string dbname, string host, int port, string userName, string password)
		{
			...
			try
			{
				...
				socket.Connect(new IPEndPoint( Dns.Resolve(host).AddressList[0], port ) );
			}
			catch (IOException ex)
			{
		...
		/// </summary>
		public void LoadDocument(string filename, string documentName)
		{
            try
            {
                Execute("DROP DOCUMENT \"" + documentName + "\"");
            }
            catch
            {
            }
